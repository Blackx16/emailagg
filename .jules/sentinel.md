...
## 2024-05-15 - Prevent Stacked Queries in Raw SQL Endpoints
**Vulnerability:** The `/api/v1/db/query` endpoint in Mission Control was vulnerable to stacked queries (e.g., executing `SELECT 1; DROP TABLE users;`) due to naive regex checks that could be bypassed using string literals or comments.
**Learning:** Using regular expressions to parse and sanitize complex SQL is an anti-pattern. Stripping strings or comments before checking for dangerous characters like semicolons introduces bypasses (like placing comment initiators inside strings). The most effective way to prevent SQL injection in naive raw query handlers without a full SQL parser is to reject any payload containing mid-statement semicolons.
**Prevention:** Strictly reject any query containing semicolons (except trailing) to prevent stacked queries. Additionally, enforce `conn.set_session(readonly=True)` using `psycopg2` to provide a defense-in-depth layer that halts any data modification attempt at the database level.
