...
class IMAPSyncService:
    ...
    async def sync(self):
        ...
        try:
            ...
            # 1. First pass: fetch headers to extract Message-IDs
            uid_to_message_id = {}
            for uid_bytes in uids:
                ...
                uid_to_message_id[uid] = message_id

            # Use bounded IN query to check existing message IDs, preventing O(N) memory leaks
            existing_message_ids = set()
            if uid_to_message_id:
                stmt_existing = select(Email.message_id).where(
                    Email.mail_account_id == self.account.id,
                    Email.message_id.in_(list(uid_to_message_id.values()))
                )
                existing_result = await self.db.execute(stmt_existing)
                existing_message_ids = set(existing_result.scalars().all())

            new_emails_count = 0
            # 2. Second pass: fetch full raw emails only for missing messages
            # Sync oldest emails first to maintain chronological notification order
            for uid_bytes in uids:
                uid = uid_bytes.decode()
                if uid not in uid_to_message_id:
                    continue

                message_id = uid_to_message_id[uid]

                # Deduplicate using the bounded existing message IDs set
