...
export default function RulesTab({ token, accounts, rules, rulesLoading, fetchData }: RulesTabProps) {
  ...
  const inputClasses = "w-full bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl px-3 py-2 text-xs text-[var(--text-primary)] focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/50 placeholder:text-[var(--text-tertiary)] transition-shadow";

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between text-left">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">Forwarding Rules</h2>
      ...
      {showAddRuleForm && (
        <form onSubmit={handleAddRule} className="p-5 rounded-xl glass border border-[var(--border)] text-left space-y-4 mt-4">
          <h4 className="text-sm font-semibold text-[var(--text-primary)] mb-2">Create New Rule</h4>

          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-[var(--text-secondary)] block">Scope (Connected Account)</label>
            <select
              value={newRuleScope}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[var(--text-secondary)] block">Subject Contains</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[var(--text-secondary)] block">From Domain</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[var(--text-secondary)] block">From Email Address</label>
              <input
                type="email"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-[var(--text-secondary)] block">Body Contains</label>
              <input
                type="text"
          ...
          <div className="space-y-1.5 pt-1">
            <label className="text-[10px] font-bold text-[var(--text-secondary)] block">Forward To (Target Email) *</label>
            <input
              type="email"
              required
          ...
          <div className="flex items-center justify-end space-x-2 pt-2">
            <button
              ...
              className="py-1.5 px-4 rounded-xl border border-[var(--border)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)] text-xs font-semibold transition cursor-pointer"
            >
              Cancel
            </button>
            <button
      ...
      {rulesLoading ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-[10px] text-[var(--text-secondary)]">Loading forwarding rules...</p>
        </div>
      ) : rules.length === 0 ? (
        <div className="p-8 text-center glass-card rounded-xl border border-[var(--border)] text-[var(--text-secondary)] text-xs">
        ...
        <div className="space-y-4 mt-4">
          {rules.map((rule) => {
            ...
            return (
              <div key={rule.id} className="p-4 rounded-xl glass-card text-left border border-[var(--border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-2.5 flex-1">
                  <div className="flex items-center space-x-2">
                    <span className={`text-[9px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded-full ${
                      rule.mail_account_id ? "bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-secondary)]" : "bg-indigo-950/60 border border-indigo-900/40 text-indigo-400"
                    }`}>
                  ...
                  <div>
                    <p className="text-xs font-bold text-[var(--text-primary)] flex items-center">
                      <span>Forward to:</span>
                      <span className="ml-1.5 text-indigo-500 underline decoration-indigo-800/40 select-all">{rule.forward_to_email}</span>
                    </p>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    {rule.condition_subject_contains && (
                      <span className="text-[10px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-md px-2 py-1 text-[var(--text-secondary)]">
                        Subject contains: <b className="text-[var(--text-primary)]">&quot;{rule.condition_subject_contains}&quot;</b>
                      </span>
                    )}
                    {rule.condition_from_domain && (
                      <span className="text-[10px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-md px-2 py-1 text-[var(--text-secondary)]">
                        From domain: <b className="text-[var(--text-primary)]">{rule.condition_from_domain}</b>
                      </span>
                    )}
                    {rule.condition_from_email && (
                      <span className="text-[10px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-md px-2 py-1 text-[var(--text-secondary)]">
                        From email: <b className="text-[var(--text-primary)]">{rule.condition_from_email}</b>
                      </span>
                    )}
                    {rule.condition_body_contains && (
                      <span className="text-[10px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-md px-2 py-1 text-[var(--text-secondary)]">
                        Body contains: <b className="text-[var(--text-primary)]">&quot;{rule.condition_body_contains}&quot;</b>
                      </span>
                    )}
                ...
                <div className="flex items-center space-x-4 self-end md:self-center">
                  <label className="flex items-center space-x-2 cursor-pointer select-none py-2">
                    <input
                      ...
                      className="accent-indigo-600 rounded bg-[var(--bg-surface)] border-[var(--border)] focus:ring-0 cursor-pointer h-5 w-5 shrink-0"
                    />
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-rose-400 hover:border-rose-950/30 hover:bg-rose-950/15 transition cursor-pointer"
                    title="Delete Rule"
                  >
