...
import { Mail, Loader2, Code, FileText } from "lucide-react";
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {selectedEmail && (
          <button
            ...
            className="md:hidden py-1 px-2.5 bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] text-[10px] font-bold rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition cursor-pointer"
          >
            ← Back to list
          </button>
        )}
      ...
      <div className="p-5 pb-28 md:pb-32 glass-card rounded-xl text-left border border-[var(--border)] max-h-[calc(100vh-180px)] overflow-y-auto flex flex-col">
        {!selectedEmail ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
            ...
            <p className="text-xs text-[var(--text-secondary)]">Select an email from the inbox list to read the message.</p>
          </div>
        ) : emailDetailLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center">
            <Loader2 className="h-6 w-6 text-indigo-500 animate-spin mb-2" />
            <p className="text-[10px] text-[var(--text-secondary)]">Retrieving full message details...</p>
          </div>
        ) : (
          <div className="space-y-4 flex-1">
            <div>
              <span className="text-[9px] uppercase tracking-wider font-black text-cyan-500 bg-cyan-950/20 border border-cyan-500/30 px-1.5 py-0.5 rounded">
                {accounts.find(a => a.id === selectedEmail.mail_account_id)?.provider || "IMAP"}
              </span>
              <h1 className="text-lg font-bold tracking-tight text-[var(--text-primary)] mt-2 leading-snug">
                {selectedEmail.subject || "(No Subject)"}
              </h1>
            </div>

            <div className="border-y border-[var(--border)] py-3 text-xs space-y-1.5 leading-relaxed text-[var(--text-secondary)]">
              <div>
                <span className="font-bold text-[var(--text-primary)]">From:</span> {selectedEmail.from_name}{" "}
                <span className="text-[var(--text-tertiary)] font-mono">&lt;{selectedEmail.from_email}&gt;</span>
              </div>
              <div>
              ...
              <div>
                <span className="font-bold text-[var(--text-primary)]">To Mailbox:</span>{" "}
                {accounts.find(a => a.id === selectedEmail.mail_account_id)?.email}
              </div>
            </div>

            {(emailDetail?.body_html || emailDetail?.body_text) && (
              <div className="flex items-center space-x-3 border-b border-[var(--border)] pb-4 mb-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all duration-200 flex items-center space-x-2 border cursor-pointer ${
                    emailBodyView === "html"
                      ? "bg-[var(--accent-muted)] border-indigo-500/50 text-indigo-500 shadow-sm"
                      : "bg-transparent border-[var(--border)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] disabled:opacity-30 disabled:cursor-not-allowed"
                  }`}
                >
            ...
            {emailDetail?.body_html && emailBodyView === "html" ? (
              <div className="rounded-xl overflow-hidden border border-[var(--border)] bg-white shadow-sm mt-2">
                <iframe
              ...
              <div className="text-xs text-[var(--text-primary)] leading-relaxed bg-[var(--bg-surface)] border border-[var(--border)] p-4 rounded-xl whitespace-pre-wrap font-mono mt-2">
                {emailDetail.body_text}
              </div>
            ) : (
              <div className="space-y-2 mt-2">
                <div className="text-xs text-[var(--text-secondary)] leading-relaxed bg-[var(--bg-surface)] border border-[var(--border)] p-3 rounded-xl whitespace-pre-wrap font-sans">
                  {selectedEmail.snippet || "No preview available."}
                </div>
                <p className="text-[10px] text-[var(--text-tertiary)] italic px-1">
